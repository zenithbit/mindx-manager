...
const FormFeedbackForStudent = () => {
  ...
  const {
    ...
    onSubmit(values) {
      const calcTP =
        getRoleTeacher === ROLE_TEACHER.ST
  ...
  const listClass: MenuProps["items"] =
    (listClassInForm.data.response?.data as Array<Obj>)?.map((item) => {
      return {
  ...
  const listGroup: MenuProps["items"] =
    (listGroupClass.data.response?.data as Array<Obj>)?.map((item) => {
      const teacher = (item.teacherRegister as Array<Obj>)?.[0]?.idTeacher;
      return {
        key: item._id,
        label: `Nhóm ${item.groupNumber} - ${
          item.locationId.locationCode
        }, MT: ${
          (item.teacherRegister as Array<Obj>)?.[0].idTeacher.fullName || ""
        }`,
  ...
  const listPoint: {
  ...
  }[] = [
    {
      label: (
        <span>
          Bạn đánh giá khả năng hỗ trợ và chăm sóc của <b>Quản lí lớp</b> tại
          MindX như thế nào?
    ...
    },
    {
      label: (
        <span>
          Bạn đánh giá <b>Cơ sở vật chất</b> tại MindX như thế nào?
          <span className="field_required">*</span>
        </span>
      ),
      name: "pointOb",
    ...
    },
    {
      label: (
        <span>
          Bạn đánh giá <b>Giảng viên</b> tại MindX như thế nào?
          <span className="field_required">*</span>
        </span>
      ),
      name: "pointST",
    ...
    },
    {
      label: (
        <span>
          Bạn đánh giá <b>Mentor (Trợ giảng)</b> tại MindX như thế nào?
          <span className="field_required">*</span>
        </span>
      ),
      name: "pointMT",
    ...
    },
    {
      label: (
        <span>
          Bạn đánh giá <b>Chương trình đào tạo</b> tại MindX như thế nào?
          <span className="field_required">*</span>
        </span>
      ),
      name: "pointSyl",
    ...
    },
    {
      label: (
        <>
          <span>
            Vui lòng chia sẻ thêm với MindX đánh giá cụ thể của bạn:
            <span className="field_required">*</span>
          </span>
          <br />
          <small>
  ...
  return (
    <div className={styles.formCollectStudent}>
      ...
      {!responseFeedback.data.success ? (
        <>
          <div
            className={`${styles.header} w-50 margin-auto bg-white mb-3 border`}
          >
            <div className={`${styles.line}`}></div>
            <div className={`${styles.contentTitle} pd-2`}>
              <h1>MINDX | KHẢO SÁT TRẢI NGHIỆM HỌC VIÊN</h1>
              <p>
                Với tinh thần cầu thị, MindX rất muốn ghi nhận{" "}
                <b>đánh giá tổng quan chất lượng khóa học của Học viên</b> mà
                chúng tôi đem lại. Để nâng cao chất lượng, MindX rất mong Học
                viên sẽ có một chút thời gian để hoàn thành khảo sát này.
              </p>
            </div>
          </div>
          <div className={`${styles.form} w-50 margin-auto`}>
            <Form onSubmit={handleSubmit}>
              {step === 1 ? (
                <>
                  <Form.Group
                    className={`${styles.mb_24} ${styles.group} ${styles.fieldInput} pd-2 radius border`}
                  >
                    <Form.Label>
                      <span>
                        Tên bạn là gì <span className="field_required">*</span>
                      </span>
                    </Form.Label>
                    <Input
                      status={
                        errors.studentName && touched.studentName ? "error" : ""
                      }
                      type="text"
                      alt=""
                      name="studentName"
                  ...
                  >
                    ...
                    <small>
                      <b>
                        <i>
                          MindX cam kết bảo mật thông tin Học viên và chỉ sử
                          dụng trong trường hợp liên hệ hỗ trợ!
                        </i>
                      </b>
                    </small>
                    <Input
                  ...
                  >
                    ...
                    {!courses.listCourse || courses.loading ? (
                      <Loading />
                    ) : getListCoures?.length > 0 ? (
                      <Radio.Group
                        className={styles.listRadio}
                        value={
                          values.course || (getListCoures[0]._id as string)
                        }
                        onChange={(e) => {
                          setFieldValue("course", e.target.value);
                          const findIdCourse = getListCoures.find(
                  ...
                  >
                    <Form.Label>
                      <span>
                        Mã lớp <span className="field_required">*</span>
                      </span>
                    </Form.Label>
                    <Dropdown
                      ...
                      onClickItem={(e) => {
                        ...
                        const getIdFeedback = (
                          listClassInForm.data.response?.data as Array<Obj>
                        )?.find((item) => {
                          return item.codeClass._id === e.key;
                        });
                        setFieldValue(
                          "feedbackId",
                          getIdFeedback!._id as string
                        );
                        setFieldValue(
                          "timeCollect",
                          (e.item as Obj)?.props.timecollect
                        );
                      }}
                      trigger="click"
                      title={
                        values.codeClass
                          ? ((
                              listClass.find(
                                (item) => item?.key === values.codeClass
                              ) as Obj
                            )?.label as string) || "Chọn mã lớp"
                          : "Chọn mã lớp"
                      }
                      icon
                      onOpenChange={(open) => {
                    ...
                    {errors.codeClass && touched.codeClass && (
                      <p className="error">{errors.codeClass}</p>
                    )}
                  </Form.Group>
                  <Form.Group
                    className={`${styles.mb_24} ${styles.group} ${styles.fieldInput} pd-2 radius border`}
                  >
                    <Form.Label>
                      <span>
                        Nhóm học tập <span className="field_required">*</span>
                      </span>
                    </Form.Label>
                    <Dropdown
                      ...
                      title={
                        values.groupNumber
                          ? values.groupNumber
                            ? ((
                                listGroup.find(
                                  (item) => values.groupNumber === item?.key
                                ) as Obj
                              ).label as string)
                            : "Chọn nhóm"
                          : "Chọn nhóm"
                    ...
                    {errors.groupNumber && touched.groupNumber && (
                      <p className="error">{errors.groupNumber}</p>
                    )}
                  </Form.Group>
                  <div
                    className={`${styles.mb_24} ${styles.group} ${styles.fieldInput} pd-2 radius border`}
                ...
                <>
                  <div
                    className={`${styles.header} margin-auto bg-white mb-3 border`}
                  >
                    <div className={`${styles.line} ${styles.line2}`}>
                      <h2>ĐÁNH GIÁ TỔNG QUAN CHẤT LƯỢNG KHÓA HỌC</h2>
                    </div>
                    <div
                      className={`${styles.contentTitle} pd-2 ${styles.contentTitle2}`}
                    >
                      <p>
                        Nội dung khảo sát bao gồm các câu hỏi về{" "}
                        <b>
                          chất lượng dịch vụ, đội ngũ giảng dạy, chương trình
                          đào tạo
                        </b>{" "}
                        để MindX tiếp tục cải thiện. Hãy đánh dấu{" "}
                        <b>mức độ hoàn thành</b> vào ô tương ứng theo{" "}
                        <b> &quot;Biểu điểm thang 5&quot;</b> như sau:
                      </p>
                      <ol>
        ...
        <div className="conGr">
          ...
          >
            ...
            <div className={`${styles.contentTitle} pd-2`}>
              ...
              <p>
                Chúng tôi rất cảm ơn bạn đã dành chút quỹ thời gian để gửi phản
                hồi! Mong rằng bạn sẽ tiếp tục có những buổi học có đầy ý nghĩa
                và nhiều sự trải nghiệm!
              </p>
              <small
