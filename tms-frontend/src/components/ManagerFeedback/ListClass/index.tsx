...
const ListClass = () => {
  ...
  const handleChangeFilter = (
    fields: string,
    value: string | number[] | boolean[] | number | boolean | Obj
  ) => {
    setFieldFilter({
      ...fieldFilter,
      [fields]: value,
  ...
  const columns: Columns = [
      ...
      filterDropdown(props) {
        return (
          <Input
            ...
            value={fieldFilter.codeClassText}
            onChange={(e) => {
              handleChangeFilter("codeClassText", e.target.value);
            }}
          />
        );
      },
          ...
          <div className={styles.selectTime}>
            <Checkbox.Group
              ...
              onChange={(checkedValue) => {
                handleChangeFilter("time", checkedValue as Array<number>);
              }}
            >
              <Checkbox value={1}>Lần 1</Checkbox>
              <Checkbox value={2}>Lần 2</Checkbox>
            </Checkbox.Group>
          </div>
        );
      },
    },
    {
      ...
      dataIndex: "date",
      title: `Ngày lấy  (${
        fieldFilter.date.month ? `Tháng ${fieldFilter.date.month}` : "Chưa chọn"
      })`,
      render(value) {
        return (
          formatDatetoString((value as Date) || new Date(), "dd/MM/yyyy") || ""
        );
      },
      filterDropdown: (props) => {
        return (
    ...
    },
    {
      ...
      filterDropdown(props) {
        return (
          <Radio.Group className={styles.option}>
      ...
      filterDropdown(props) {
        return (
          <div className={styles.selectTime}>
            <Checkbox.Group
              className={styles.checkboxGroup}
              defaultValue={fieldFilter.enabled}
              onChange={(checkedvalue) => {
                handleChangeFilter("enabled", checkedvalue as Array<boolean>);
              }}
            >
              <Checkbox value={true}>Đã triển khai</Checkbox>
              <Checkbox value={false}>Chưa triển khai</Checkbox>
            </Checkbox.Group>
          </div>
        );
      },
        ...
        return (
          <div className={styles.actionChecked}>
            <CheckCircleOutlined
              ...
              className={`${value ? styles.active : styles.deactive} ${
                styles.iconCheck
              }`}
            />
            <CloseCircleOutlined
      ...
      filterDropdown(props) {
        return (
          <Radio.Group
            ...
            defaultValue={fieldFilter.done}
            onChange={(e) => {
              handleChangeFilter("done", Boolean(e.target.value));
            }}
          >
  ...
  const rowData: RowData[] =
    (dataListClass.data.response?.data as Array<Obj>)?.map((item) => {
      return {
        ...item,
