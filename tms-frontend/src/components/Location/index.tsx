...
const columns: Columns = [
  ...
  },
  {
    ...
    render(value) {
      return (
        <div
          className={`${value ? "bgRunning" : "bgStop"} ${styles.statusActive}`}
        >
...
const Location = () => {
  ...
  const area: TabsProps["items"] = [
    ...
    ...(listArea.data.response && listArea.data.response.data
      ? (listArea.data.response.data as Obj[]).map((item) => {
          return {
  ...
  const rowData: RowData[] =
    (locations.locations?.data as Obj[])?.map((item) => {
      return {
        key: item._id as string,
        ...item,
      };
