...
const ManagerClass = () => {
  ...
  const mapDataListClass: RowData[] =
    ((listClass.response?.data as Obj)?.classes as Array<Obj>)?.map((item) => {
      let crrST = "";
  ...
  const getListClTeacherPoint = listClassTeacherPoint.data.response
    ?.data as Array<Obj>;
  ...
  const columns: Columns = [
    {
      ...
      render(value, record, index) {
        const filterClTeacherPoint: Array<Obj> = getListClTeacherPoint?.filter(
          (item) => {
        ...
        return (
          <span>
            {value}{" "}
            {filterClTeacherPoint && (
              <b
                style={{
                  color: getColorTeacherPoint(
                    Number(
                      filterClTeacherPoint[filterClTeacherPoint.length - 1]
                        ?.teacherPoint
                    )
                  ),
                }}
  ...
  const handleClickRow = (record: Obj) => {
    ...
    router.push(
      hasRole
        ? `/te/manager/class/detail/${record.key}`
        : `/teacher/class/detail/${record.key}`
    );
  };
