...
const ListTE = () => {
  ...
  const columns: Columns = [
    ...
    },
    {
      ...
      render(value, record) {
        return `${value}${
          record.courseId ? `-${(record.courseId as Obj)?.courseName}` : ""
        }`;
      },
    },
    {
      ...
      render(value) {
        return (
          <div
            className={`${value ? "bgRunning" : "bgStop"} ${
              styles.statusActive
            }`}
          >
  ...
  const rowData: RowData[] = getListTe
    ?.filter((item) => {
  ...
  useEffect(() => {
    listTe.query({
      query: {
        fields:
          "_id,teName,positionTe,courseId,courseName,email,phoneNumber,activate,img",
        getAll: true,
      },
    });
  }, []);
