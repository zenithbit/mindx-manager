...
const TableRecruitment = () => {
  const { modal, pagination, conditionFilter, isSearch } =
  ...
  useEffect(() => {
    ...
    if (
      !listDataRecruitment.data.response ||
      (getPayloadQuery &&
  ...
  const columns: Columns = [
      ...
      render(value, record) {
        return (
          <div
            ...
            style={{
              backgroundColor:
                getColorByResultInterview[value as ResultInterview],
            }}
          >
    ...
    },
    {
      ...
      dataIndex: "sendMail",
      render(value, record) {
        return record.result === ResultInterview.PASS
          ? "Đã xong"
          : !value
          ? "Chưa gửi"
          : "Đã gửi";
  ...
  return (
    <div className={styles.tableView}>
      <Table
        ...
        showSizePage
        maxPage={
          (listDataRecruitment.data.response?.data as Obj)?.totalPage as number
        }
      />
      {modal.config.isShow && (
        <Popup
